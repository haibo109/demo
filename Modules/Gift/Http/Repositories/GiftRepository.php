...
{
    public static function Factory(): GiftRepository
    {
        return new self();
    }
    ...
    public function doGiveGroupGift($uid, $to_uid, $giftInfo, $scene, $room_id, $number_group, $eachUserGiftCoin, $giftUnitCoin)
    {
        try {
            return DB::transaction(function () use ($room_id, $uid, $to_uid, $giftInfo, $scene, $number_group, $eachUserGiftCoin, $giftUnitCoin) {
                ...
                # 上礼物墙
                # 调用队列进行礼物墙处理
                $this->$this->addQueueHandle(['addGiftWall', ($to_uid, $giftInfo['id']]);
...
//                }
                #此处记录收历任返币张单队列处理
                $this->addQueueHandle(['UserBill', $giftUnitCoin, $to_uid, $room_id]);

                # 更新 房间排行榜 排行榜数据
                $this->addQueueHandle(['giveGroupGiftUpdateRankings', $room_id, $uid, $to_uid, $giftInfo, $eachUserGiftCoin]);
                //RankingDataRepository::Factory()->giveGroupGiftUpdateRankings($room_id, $uid, $to_uid, $giftInfo, $eachUserGiftCoin);

                # 更新用户在房间内的贡献币数和收益币数
                $this->addQueueHandle(['updateUserRoomIncomeAndExpend', $room_id, $uid, $to_uid, $eachUserGiftCoin]);
...
//                    VcrRoom::query()->where('id', $room_id)->increment('spend_money', $giftBillInfo['gift_money']);
                    # 更新房间总消费金币和金额操作添加到队列
                    $this->addQueueHandle(['updateCoinMoney', $room_id, $giftBillInfo]);
                }

                # 记录礼物收益分配
                # 将处理逻辑添加到队列处理

                $this->addQueueHandle(['giftGroupIncomeDistribution', $eachUserGiftCoin, $room_id, $bill_id, $to_uid]);
     ...
     * 添加队列处理
     */
    public function addQueueHandle($data = [])
    {
        #此处进行调用队列处理
